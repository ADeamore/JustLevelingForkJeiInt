...
public class HandlerAptitude {
    ...
    private static Map<String, List<com.seniors.justlevelingfork.client.core.Aptitudes>> Enchants;
    ...
    public static void UpdateLockEnchant(List<LockEnchant> lockEnchants) {
        Map<String, List<Aptitudes>> aptitudeMap = new HashMap<>();

        for (LockEnchant lockEnchant : lockEnchants) {
            List<Aptitudes> aptitudesList = new ArrayList<>();
            Iterator<LockEnchant.AptitudeLevel> enchantiterator = lockEnchant.Aptitudes.iterator();
            while(enchantiterator.hasNext()) {
                LockEnchant.AptitudeLevel aptitude = enchantiterator.next();
                if (aptitude.Aptitude == null) {
                    continue;
                }
                Aptitude aptitudeName = RegistryAptitudes.getAptitude(aptitude.Aptitude.toString());
                ...
                aptitudesList.add(new Aptitudes(aptitude.Aptitude.toString(), lockEnchant.Enchant, false, aptitudeName, aptitude.Level));
            }
            if (aptitudesList.isEmpty()) {
                continue;
            }
            aptitudeMap.put(lockEnchant.Enchant, aptitudesList);
        }

        // Replace the old aptitudes map so client lock items doesn't affect while playing on server.
        Enchants = aptitudeMap;
    ...
    public static Map<String, List<Aptitudes>> getEnchantAptitudes() {
        Map<String, List<Aptitudes>> aptitudeMap = new HashMap<>();
        List<LockEnchant> lockEnchantList = HandlerLockEnchantsConfig.HANDLER.instance().lockEnchantList;

        for (LockEnchant lockEnchant : lockEnchantList) {
            List<Aptitudes> aptitudesList = new ArrayList<>();
            for (LockEnchant.AptitudeLevel aptitude : lockEnchant.Aptitudes) {
                if (aptitude.Aptitude == null) {
                    JustLevelingFork.getLOGGER().warn("Enchant {} with wrong aptitude (APTITUDE NOT FOUND), Skipping...", lockEnchant.Enchant);
                    continue;
                }
                Aptitude aptitudeName = RegistryAptitudes.getAptitude(aptitude.Aptitude.toString());
                if (aptitudeName == null) {
                    JustLevelingFork.getLOGGER().warn("Enchant {} with wrong aptitude (APTITUDE \"{}\" NOT FOUND), Skipping...", lockEnchant.Enchant, aptitude.Aptitude.toString());
                    continue;
                }
            ...
            if (aptitudesList.isEmpty()) {
                JustLevelingFork.getLOGGER().warn("Enchant {} with no aptitudes (ITEM WITH NO APTITUDES), Skipping...", lockEnchant.Enchant);
                continue;
            }
            aptitudeMap.put(lockEnchant.Enchant, aptitudesList);
        }
    ...
    public static List<Aptitudes> getEnchantValue(String key) {
        if (Enchants == null) {
            Enchants = getEnchantAptitudes(); // Cache the items with their respective aptitudes, so it doesn't try to read the items every time (:
        }
